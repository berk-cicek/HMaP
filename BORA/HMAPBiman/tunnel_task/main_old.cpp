...
int main(int argc, char* argv[]) {
    ...
    rai::Configuration C;
    C.addFile("HMAP_tunnel_env.g");  

    rai::Configuration C2;
    C2.addFile("HMAP_tunnel_actuated_env.g");  
    ...
    C.getFrame("l_panda_link0")->setPose(rai::Transformation(rai::Vector(0.1, -0.1, .1), rai::Quaternion(0.7073883, 0., 0., 0.7068252)));
    C.getFrame("r_panda_link0")->setPose(rai::Transformation(rai::Vector(-0.6, -0.1, .1), rai::Quaternion(0.7073883, 0., 0., 0.7068252)));
    C.view(true, "Initial Configuration");

    const arr qHome = C.getJointState();
    const arr qF = {0.5, 0.25, 0.09001, 1, 0, 0, 0};
    ...
    double filter = 0.15; // A percentage value
    double threshold = 3.7; // Cost threshold
    bool is_tool_aval = false;
    std::string tool = "";
...
std::shared_ptr<SolverReturn> moveSkeleton(rai::Configuration& C, const std::string& gripper, const std::string& target, const std::string& tool, const std::string& contact_point, const std::string& waypoint, bool& is_aval_l, bool& is_aval_r, const double threshold, const double calib, const bool isTrial) {
    ...
    if((gripper == "l_l_gripper" && !is_aval_l) || (gripper == "r_l_gripper" && !is_aval_r)){
        ...
        komo_path->addObjective({1.5, -1}, FS_poseDiff, {gripper.c_str(), contact_point.c_str()}, OT_sos, {1e2});        
    } else {
        ...
        komo_path->addObjective({1, -1}, FS_poseDiff, {gripper.c_str(), contact_point.c_str()}, OT_sos, {1e2});
    }
