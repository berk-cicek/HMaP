...
int main(int argc, char* argv[]) {
    ...
    // Open a file stream to write to a .g file
    std::ofstream outFile("HMAP_bookshelf_conf.g");

    // Check if the file stream is open
    if (outFile.is_open()) {
        // Write the contents of C to the file
        outFile << C << std::endl;

        // Close the file stream
        outFile.close();
        std::cout << "Successfully wrote to output.g" << std::endl;
    } else {
