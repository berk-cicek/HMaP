...
void randomizeShelf(rai::Configuration& C, rai::Configuration& C2, StringA& all_frames);
...
int main(int argc, char* argv[]) {
    ...
    C_copy.addFile("../../HMAP/config/bookshelf/HMAP_bookshelf_conf.g"); 

    rai::Configuration C2;
    rai::Configuration C2_copy;
    C2.addFile("../../HMAP/config/bookshelf/HMAP_bookshelf_actuated_conf.g");  
    C2_copy.addFile("../../HMAP/config/bookshelf/HMAP_bookshelf_actuated_conf.g");
    ...
    for(uint i = 0; i < sample_count; i++) {
        ...
        addMarker(C, {qF(0), qF(1), qF(2)}, "wp", "shelf", 0.2, false, {qF(3), qF(4), qF(5), qF(6)});
        ...
        // Randomize the shelf
        randomizeShelf(C, C2, all_frames);
        
        C2.setJointState(C.getFrame("box")->getPose());
        qF = C.getFrame("wp")->getPose();
        q_obs = C.getFrame("q_obs")->getPose();

        C2.view(false, "Randomized Configuration");

        // Generate waypoints
        HMAPBiman hmap_biman(C, C2, qF, q_obs, target, total_obstacle_count, tool_list, gripper_list, filter, video_path, waypoint_factor, false);
        if(hmap_biman.run()){
